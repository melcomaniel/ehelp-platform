...
- Add a Platform Administrator-only Organization Admins directory with search, status/invitation filters, pagination, and links back to each organization detail.
...
- Revalidate account and organization status on login, session lookup, and protected server operations so suspended accounts and suspended or archived tenants cannot use existing sessions.
...
- **Web:** Next.js App Router pages below `/admin/organizations` and `/admin/organization-admins`, built with the existing components and `/api/nest` authenticated proxy.
...
- **Authorization/security:** all organization-management handlers require an active, unscoped `PLATFORM_ADMIN`; protected operations reload the account and tenant lifecycle state. Suspended accounts receive `Account is suspended` and stale web cookies are cleared. Platform Administrator access to tenant business endpoints is denied server-side.
